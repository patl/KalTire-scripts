...
Then(/^select Year for tires by vehicle Packages$/) do
  sleep (2)
  @browser.element(:xpath, "(//a[contains(text(),"+(@year)+")])[6]").click
end

Then(/^select Make for tires by vehicle Packages$/) do
  sleep (2)
  @browser.element(:xpath, "html/body/div[1]/div[4]/div[2]/div[2]/div/div/div[2]/div/ul/li[3]/ul/li/div/div[3]/div[4]/div[2]/div/ul/li/div/ul["+(@make_column)+"]/li[" +(@make_row)+"]/a").click
  sleep(2)
  selected_make = @browser.element(:xpath, "html/body/div[1]/div[4]/div[2]/div[2]/div/div/div[2]/div/ul/li[1]/ul/li/div/div[3]/div[1]/div[2]/a").text
  @selected_make =selected_make
end
...
Then(/^select Size for tires by vehicle Packages$/) do
  ...
  begin
    ...
    if size == true
      sleep (1)
      @browser.element(:xpath, "html/body/div[1]/div[4]/div[2]/div[2]/div/div/div[2]/div/ul/li[3]/ul/li/div/div[3]/div[4]/div[6]/div/ul/li[1]/a")
    else
      puts 'nothing select'
    end
  rescue Selenium::WebDriver::Error::NoSuchElementError
...
verify { (@browser.element(:css, "h1.page-title").text).should == "Shop wheels" }
@browser.element(:id, "headerNavLocationCTA").click
@browser.element(:id, "locationChangeInput").clear
@browser.element(:id, "locationChangeInput").send_keys "alberta"
verify { element_present?(:id, "add-to-cart").should be_true }
@browser.element(:id, "add-to-cart").click
verify { (@browser.element(:css, "h4.popup-title").text).should == "Added to cart" }
verify { element_present?(:xpath, "//div[@id='cart-popup-container']/div[2]").should be_true }
@browser.element(:link, "Select a different wheel").click
@browser.element(:link, "Shop for tires").click
verify { (@browser.element(:css, "a.pk-edit-wheel").text).should == "(Edit)" }
verify { (@browser.element(:css, "div.circle-indicator.selected").text).should == "2 Select a Tire" }
@browser.element(:link, "G-FORCE COMP-2 A/S").click
@browser.element(:id, "add-to-cart").click
verify { (@browser.element(:css, "h1").text).should == "Your Cart" }


Then(/^user selects OEM size for packages$/) do
  @browser.element(:class, "oe-size").click
end

And(/^verify that CLP is displayed for selected size SHOP WHEELS$/) do
  @browser.element(:css, "span.page-title").text == "Shop Wheels".upcase
  @browser.element(:css, "div.circle-indicator.selected").text == "1 Select a Wheel"
end

Then(/^user press on the first wheel product from the grid$/) do
  @browser.element(:xpath, "//li[@class='grid-tile small-12 medium-4 columns new-row']//div[2]/div[2]/a[2]").click
end